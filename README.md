<div align="center">

# 🚀 AI快侠

### AI驱动的全平台视频下载神器

[![Version](https://img.shields.io/badge/version-1.0.0-blue.svg)]()
[![Platform](https://img.shields.io/badge/platform-iOS%20%7C%20Android%20%7C%20macOS%20%7C%20Windows%20%7C%20Linux-brightgreen.svg)]()
[![License](https://img.shields.io/badge/license-proprietary-red.svg)]()

**粘贴文字，AI自动识别链接，一键下载全网视频**

### 🌐 访问首页：[https://opc007.github.io/ai-kuaixia/](https://opc007.github.io/ai-kuaixia/)

[🚀 马上使用](https://opc007.github.io/ai-kuaixia/landing/app.html) · [✨ 功能](#-核心功能) · [📖 使用方法](#-使用方法) · [❓ FAQ](#-faq)

[![Deploy to Render](https://render.com/images/deploy-to-render-button.svg)](https://render.com/deploy?repo=https://github.com/opc007/ai-kuaixia)

</div>

---

## ✨ 核心功能

### 🤖 AI智能识别
直接粘贴抖音、快手的分享文字（包含文字+链接的整段），AI自动提取视频链接，无需手动清理。

### 🎯 全平台覆盖
支持 **抖音、快手、B站、小红书、YouTube、TikTok** 等10+主流平台，持续扩展中。

### 🔒 突破下载限制
即使创作者设置了"不允许下载"，AI快侠也能智能解析并下载视频内容。

### 💬 AI对话助手
内置AI对话功能，不只是下载工具，更是你的智能视频助手。

### 📱 多端支持
- 🍎 iOS / Android 手机端
- 💻 macOS / Windows / Linux 桌面端
- 🌐 浏览器在线使用

---

## 📖 使用方法

**超简单，三步搞定：**

```
1️⃣ 复制分享文字
   例如：「卷疯了！61 键满配 + RGB 光效... https://v.douyin.com/xxx 复制此链接，打开【抖音】...」

2️⃣ 粘贴到AI快侠
   AI自动识别并提取视频链接

3️⃣ 点击下载
   视频自动保存到本地
```

---

## 📥 下载

> **部署说明**：本仓库含网页 + 后端源码，详见 [DEPLOY.md](./DEPLOY.md)  
> 网页托管 GitHub Pages，API 可一键部署到 [Render](https://render.com) 免费版，后续可迁移阿里云。

| 平台 | 状态 | 下载 |
|------|------|------|
| 🌐 网页版 | ✅ 可用 | [立即使用](https://opc007.github.io/ai-kuaixia/landing/app.html) |
| 🍎 iOS | 🔜 即将上线 | - |
| 🤖 Android | 🔜 即将上线 | - |
| 💻 macOS | ✅ 可用 | [下载 .dmg]() |
| 🖥️ Windows | 🔜 即将上线 | - |
| 🐧 Linux | 🔜 即将上线 | - |

---

## 🎬 支持的平台

<div align="center">

| 平台 | 支持状态 |
|:---:|:---:|
| 🎵 抖音 | ✅ |
| 🎬 快手 | ✅ |
| 📺 B站 | ✅ |
| 📕 小红书 | ✅ |
| ▶️ YouTube | ✅ |
| 🎶 TikTok | ✅ |
| 📹 微视 | ✅ |
| 🍉 西瓜视频 | ✅ |
| 🌐 其他网站 | ✅ |

</div>

---

## ❓ FAQ

**Q: 为什么有些视频下载不了？**
A: 部分视频可能因平台限制或网络问题暂时无法下载，请稍后重试。

**Q: 下载的视频有水印吗？**
A: 大部分平台支持无水印下载，部分平台因技术限制可能保留水印。

**Q: 需要付费吗？**
A: 基础功能免费使用，高级功能需要充值积分。

**Q: 支持批量下载吗？**
A: 暂不支持，后续版本会加入此功能。

---

<div align="center">

**⭐ 如果觉得好用，请给我们一个 Star！⭐**

🌐 访问首页：[https://opc007.github.io/ai-kuaixia/](https://opc007.github.io/ai-kuaixia/)

© 2026 AI快侠. All rights reserved.

</div>
